...
# ─────────────────────────── log writer ──────────────────────────────
#
# PARITY_PROGRESS.md is grouped by MATLAB-doc section and lists every
# function from MATLAB_PARITY_TODO.md (regenerated by
# regenerate_progress.py). Each spec run updates the row(s) for its
# function in place — same function may appear in multiple sections
# (e.g. interpft is listed under both Interpolation and Fourier);
# every occurrence gets the same fresh measurement.
...
def update_row(*, name: str, nk: Result | None,
    ...
    if touched == 0:
        ...
        if misc_marker not in new_text:
            ...
            new_text += "Functions benched by the harness that don't appear in `MATLAB_PARITY_TODO.md`. Move them into a real section once added there.\n\n"
