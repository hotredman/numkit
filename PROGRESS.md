...
| `corr` | ⚠️ | 0.008 | 226.02× |  | OK | Sig: c = corr(X); c = corr(X, Y). Pearson correlation matrix (single-arg = between-cols-of-X; two-arg = X.cols × Y.cols matrix). Two-arg form added cycle 84. Type/Rows/Tail/Weights NV-pairs accept-and-ignore (Pearson only). |
