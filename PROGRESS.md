...
| `fspecial` | ✅ 🔬 | Sig: h = fspecial(type, params). DEEP-PROBE 2026-05-31 (c160): fspecial('disk',r) now computes MATLAB R2025b's EXACT sub-pixel area-coverage integral (the fraction of each unit cell inside the disk of radius r), not the previous linear-taper approximation (self-admitted 'Approximate area inside disk'). Bit-exact vs MATLAB across r=1..10: disk(2) is 5x5, sum=1, centre h(3,3)=0.0795774715, corner h(1,1)=0, mid-edge h(3,1)=0.0381149714, h(2,2)=0.0783813542; disk(3) centre h(4,4)=0.0353677651; disk(5) is 11x11 centre h(6,6)=0.0127323954. Also fixed the kernel SIZE for non-integer r (now 2*ceil(r-0.5)+1 per MATLAB). 'average'/'gaussian'/'laplacian'/'log'/'sobel'/'prewitt' unchanged. covers:[fspecial]. |
...
| `imgradient` | ✅ 🔬 | imgradient/imgradientxy sobel+prewitt gradient SIGN vs MATLAB R2025b. DEEP-PROBE 2026-05-31 (toolbox pivot, libs/image): the 2-D sobel/prewitt kernels were sign-flipped (kx=[1 0 -1;...]) so imgradientxy returned negated Gx/Gy (Gx=-40 vs MATLAB +40 for a left->right-increasing ramp) and imgradient's Gdir=atan2(-Gy,Gx) was 180 deg off (168.69 vs MATLAB -11.31). Gmag was unaffected (sign-invariant); 'central'/'intermediate' were already correct; the 3-D imgradientxyz kernels were already MATLAB-signed. Fix flips kx/ky to MATLAB's hx=[-1 0 1;-2 0 2;-1 0 1]. reshape(1:25,5,5) at (3,3): sobel Gmag=40.7921563, Gdir=-11.3099325, Gx=40, Gy=8; prewitt Gdir=-11.3099325, Gx=30, Gy=6. The 2-arg imgradient(Gx,Gy) input form remains deferred. covers:[imgradient,imgradientxy]. |
| `imgradientxy` | ✅ 🔬 | imgradient/imgradientxy sobel+prewitt gradient SIGN vs MATLAB R2025b. DEEP-PROBE 2026-05-31 (toolbox pivot, libs/image): the 2-D sobel/prewitt kernels were sign-flipped (kx=[1 0 -1;...]) so imgradientxy returned negated Gx/Gy (Gx=-40 vs MATLAB +40 for a left->right-increasing ramp) and imgradient's Gdir=atan2(-Gy,Gx) was 180 deg off (168.69 vs MATLAB -11.31). Gmag was unaffected (sign-invariant); 'central'/'intermediate' were already correct; the 3-D imgradientxyz kernels were already MATLAB-signed. Fix flips kx/ky to MATLAB's hx=[-1 0 1;-2 0 2;-1 0 1]. reshape(1:25,5,5) at (3,3): sobel Gmag=40.7921563, Gdir=-11.3099325, Gx=40, Gy=8; prewitt Gdir=-11.3099325, Gx=30, Gy=6. The 2-arg imgradient(Gx,Gy) input form remains deferred. covers:[imgradient,imgradientxy]. |
...
| `poly2label` | ❌ |  |
| `regionprops` | ✅ 🔬 | regionprops intensity-image form vs MATLAB R2025b. DEEP-PROBE 2026-05-31 (toolbox pivot, libs/image): regionprops(BW,I,props) was unsupported — a numeric 2nd arg threw 'property names must be strings'. Added the grayscale-image dispatch (numeric 2nd arg = intensity image) plus the intensity fields: MeanIntensity=mean(I over region) (double); MaxIntensity/MinIntensity=max/min(I); WeightedCentroid=sum(I*[x y])/sum(I); PixelValues=column vector of I values in PixelIdxList (column-major) order. Region {(2,2),(2,3),(3,3)} with I=reshape(1:16,4,4): pixel values [6;10;11] -> Mean=9, Max=11, Min=6, WeightedCentroid=[75/27 65/27]=[2.7777778 2.4074074]. uint8 image: Mean=80, Max=100. A string/cell 2nd arg is still a property; intensity fields are NOT in the basic default set. covers:[regionprops]. |
| `regionprops3` | ❌ |  |
...
| `robustfit` | ✅ 🔬 | robustfit default intercept term vs MATLAB R2025b. DEEP-PROBE 2026-05-31 (toolbox pivot, libs/stats): robustfit treated X as the full design matrix and never prepended the constant column, so robustfit(x,y) returned a single through-origin coefficient instead of MATLAB's [intercept; slope]. MATLAB adds the constant by default (const='on'); 'off' suppresses it. Fix prepends a ones column unless const='off'. With the outlier fully downweighted by the bisquare weight the fit is exact: y=2x+1+outlier -> b=[1;2]; y=3+2x+0.5x^2+outlier -> [3;2;0.5]; const 'off' -> 1 coefficient. (Residual leverage-adjustment precision on NOISY data is a separate known gap; tested here only on exact-recovery data.) covers:[robustfit]. |
| `lscov` | ✅ | Sig: [x, stdx, mse, S] = lscov(A, b[, w]). Weighted least squares. mse = SSR/(N-p); S = mse·(A'WA)^(-1). Full N×N covariance V deferred (errors). Bit-identical to MATLAB R2025b on OLS and weighted paths. |
...
| `normalize` | ✅ 🔬 | normalize [N,C,S] centering/scaling outputs vs MATLAB R2025b. DEEP-PROBE 2026-05-31 (non-string pivot): normalize returned only N; the 2nd (C, centering) and 3rd (S, scaling) outputs were missing -> [N,C,S]=normalize(...) errored 'Undefined variable C'. MATLAB: N == (A-C)./S, one C/S value per operating slice (1x1 for a vector, 1xW for a matrix, column-wise). Per method: zscore C=mean S=std (C=4 S=2 for [2 4 6]); range C=min S=max-min (2,4); norm C=0 S=vecnorm (0,5); center C=mean S=1 (4,1); scale C=0 S=std (0,2); a 2x3 matrix -> C=[2.5 3.5 4.5] S=[2.1213...] (1x3). Fix: capture cVal/sVal in each method branch of normalize() and expose them via two optional out-pointers; normalize_reg wires outs[1]/outs[2] when nargout>=2/3. The 1-output path is unchanged. NOTE: for a CUSTOM 'range' [lo hi] the affine offset is not captured by (A-C)./S (C/S are the data min/span, exact for the default [0 1]) — documented MATLAB limitation. covers:[normalize]. |
