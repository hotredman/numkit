...
| `griddedinterpolant` | ❌ |  |  |
| `interp1` | ✅ | OK | Sig r=interp1(x,y,xq[,method]). linear (default), nearest, spline, pchip, makima, and 'previous'/'next' step interpolation. 'next' = value at smallest knot >= xq; 'previous' = largest knot <= xq; queries outside [x(1),x(end)] -> NaN. interp1([1 2 3],[10 20 30],1.5,'next')=20, 'previous'=10; @knot both=knot value; 3.5 'previous'=NaN. numkit previously errored 'unknown method' on next/previous -- added. Matches MATLAB R2025b. |
...
| `medfilt1` | ✅ | OK | Sig: r = medfilt1(...). Spec-extension batch 2026-05-09 (signal namespace). |
