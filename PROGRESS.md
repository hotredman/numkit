...
| `audio_pitch_harmonics` | — | 92.463 | 0.55× |  | OK | MATLAB Audio Toolbox cycles E + F + K + K-2: pitch + harmonicRatio. pitch via NCF (Normalized Cross-correlation Function) — default method per MATLAB R2025b. Defaults match MATLAB: WindowLength=round(0.052*fs)=832 samples for fs=16000, OverlapLength=round(0.042*fs)=672, hop=160, Range=[50,400] Hz. harmonicRatio uses 30/20ms windows. Frame counts (95 for pitch, 98 for harmonicRatio at 16001 samples) match MATLAB exactly. f0 NCF within ~1% of true frequency. Cycle F: harmonicRatio gained MATLAB R2025b parity (auto low-edge + Smith parabolic refinement). Cycle K: Method='CEP' (Cepstrum, Noll 1967) — bit-equal via batched FFT. Cycle K-2: Method='PEF' (Pitch Estimation Filter, Gonzalez & Brookes 2011) — bit-equal MATLAB R2025b audio.internal.pitch.PEF.m. Pipeline: log-spaced frequency grid (NFFT points from 10 Hz to min(fs/2-1,4000)), interp1 power spectrum onto log grid, FFT-based cross-correlation with PEF filter (h = 1/(γ - cos(2π q)) - β where q = logspace(log10(0.5), log10(K+0.5), NFFT/2)), peak picking → f0 = logSF(peak). All FFT sizes used by PEF/CEP are power-of-2 by construction. KNOWN GAPs: 'LHS'/'SRH' still deferred (use fftLength=round(fs) which is non-power-of-2 typically — blocked on libs/signal::fft fix); pitchnn deferred (needs DNN runtime). Tolerance 5% for parity (algorithmic equivalence rather than bit-equality for NCF; CEP and PEF are bit-equal). |
...
| `signal_modulate` | — | 0.008 | 239.31× |  | OK | MATLAB Signal Toolbox modulate (Phase 4.12) — analog modulation. Supports am/amdsb-sc/amdsb-tc/fm/pm modes (the most common 4 of 8). Per-element formulas: am = x·cos(2πFct); amdsb-tc = (x-offset)·cos(...); fm = cos(2πFct + kf·cumsum(x)); pm = cos(2πFct + kp·x). Default kf = (Fc/Fs)·2π/max|x|, kp = π/max|x|, offset = min(x). Bit-equal MATLAB R2025b on 7/7 fingerprints across all 4 modes. KNOWN GAPs: amssb (uses hilbert), pwm/ptm/ppm (specialised pulses), qam (complex carrier) deferred. Octave 11.1.0 doesn't ship modulate in core (signal package only). |
| `signal_demod` | — | 0.013 | 800.44× |  | OK | MATLAB Signal Toolbox demod (Phase 4.13) — analog demodulation. Implements am / amdsb-sc (alias) / amdsb-tc; uses 5th-order Butterworth lowpass + filtfilt per MATLAB demod.m. Approximate-equal MATLAB R2025b on 8 fingerprints (tol 0.01); ~2e-3 diff vs MATLAB attributed to filtfilt initial-condition edge handling. KNOWN GAPs: fm/pm modes use hilbert (blocked on libs/signal::fft sign-convention bug — same blocker as Cycle J/LHS/SRH); amssb/pwm/ptm/ppm/qam similarly deferred. Octave 11.1.0 doesn't ship demod in core (signal package only). |
