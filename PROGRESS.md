...
| `roifilt2` | ✅ | 0.005 | 1.00× |  | OK | Sig: J = roifilt2(h, I, BW) (filter form) or roifilt2(I, BW, fun) (function-handle form) (cycle 98). Form 1: imfilter(I,h) (correlation, zero boundary, same) with only BW pixels replaced; output = I elsewhere; class=class(I). Form 2: apply fun to whole I, keep only BW pixels; output class follows fun's result. Detection: arg3 funchandle/string → form 2, else form 1. fun dispatched via Engine::callFunctionHandle. Bit-equal MATLAB R2025b on magic(6) with 3×3 Laplacian + averaging (form 1) and uint8 x*2 / double+0.5 handles (form 2) — image_roifilt2.json. KNOWN GAP (upstream, not roifilt2): even-size filters inherit numkit imfilter's even-kernel anchoring, which differs from MATLAB by one pixel — a pre-existing imfilter gap tracked separately; odd filters (the ROI norm) are bit-exact. Octave ships it in image pkg only → N/A. |
