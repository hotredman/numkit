...
| `sort` | ✅ | 0.008 | 10.25× |  | OK | Sig [s,i]=sort(X[,dim][,direction]): default ASCEND with NaN LAST: sort([3 1 NaN 2])=[1 2 3 NaN], idx=[2 4 1 3]. 'descend' with NaN FIRST: [NaN 3 2 1], idx(1)=3. Matrix sort(M,1,'descend') sorts each column descending: col1 [8 3 1], col2 [5 4 2]. COMPLEX sort orders by magnitude |z| then phase angle arg(z): sort([3+4i 1 5i])=[1 3+4i 5i] idx [2 1 3]; 'descend' reverses -> 5i first, 1 last; sort([2 -2 1i -1i])=[-1i 1i 2 -2] (mag 1s before mag 2s, angle ties). numkit previously: 'descend' ignored + NaN unsorted (fixed earlier); COMPLEX sort threw 'Not a double array' (added sortComplex 2026-05-29). |
...
| `median` | ✅ | 4.569 | 1.09× |  | OK | Sig: M = median(X). 1M-pt full sort + middle. 50 iters. Scalar fp. DEEP-PROBE 2026-05-29: median of an EMPTY array -> NaN (not 0x0): median([])=NaN scalar; median(zeros(0,3))=[NaN NaN NaN] (1x3); median(zeros(3,0))=1x0. |
...
| `mat2str` | — | 0.015 | 140.98× |  | OK | Sig: s = mat2str(X[, n]). Char-code fingerprints (string == comparison gives a vector → use double(s(k))). Covers real matrix + complex scalar/row/col + a purely-imaginary element ('0-3i', no '-0') + all-zero-imag complex printed as real. Queue-clearing 2026-05-29: mat2str on a COMPLEX array previously threw 'Not a double array' / 'Cannot convert complex to scalar'; now formats re±|im|i. |
