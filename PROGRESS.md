...
| `audio_scales` | — | 0.011 | 207.54× |  | OK | MATLAB Audio Toolbox frequency-scale + loudness conversions (cycle A): hz2mel/mel2hz (O'Shaughnessy default), hz2bark/bark2hz (Traunmüller 1990 with low/high-frequency corrections; bark2hz uses the asymmetric 26.28 denominator from MATLAB's bark2hz.m source), hz2erb/erb2hz (Glasberg & Moore 1990 with constants log(10)*1000/(24.673*4.368) and 0.004368 — extracted bit-for-bit from MATLAB hz2erb.m), phon2sone/sone2phon (ISO 532-1 default with smooth break at 40 phon = 1 sone). Bit-equal with MATLAB R2025b on all 20 fingerprint points. Octave 11.1.0 doesn't ship these in core (Audio package only; Octave's audio package missing these specifically). |
...
| `audio_cepstral` | — | 6.600 | 1.78× |  | OK | MATLAB Audio Toolbox cycle D + G: cepstralCoefficients (bit-equal — DCT-II unitary matrix from createDCTmatrix.m + log10 rectification, output shape M×NumCoeffs). Tested on single-frame and multi-frame synthetic 8-band spectra. Cycle G upgrade: mfcc now BIT-EQUAL with MATLAB R2025b — uses Slaney band edges (audio.internal.slaneybandedges, 42 entries: 13 linear at 66.66 Hz step + 29 log-spaced ratio 1.0711703), Slaney-style designMelFilterBank ('Hz' designDomain, 'Bandwidth' normalization, 'linear' FilterBankDesignDomain), |FFT| magnitude (NOT |FFT|^2 power), natural-log per-frame energy of UNWINDOWED signal as first column ('append' LogEnergy default), then log10+DCT-II via cepstralCoefficients. Output: numFrames × (NumCoeffs+1). KNOWN GAPs: gtcc still aliases to mfcc (gammatone ERB IIR filterbank deferred to v2). Octave 11.1.0 doesn't ship cepstralCoefficients/mfcc/gtcc in core (Audio package only). |
| `audio_pitch_harmonics` | — | 36.534 | 0.50× |  | OK | MATLAB Audio Toolbox cycle E (FINAL audio cycle): pitch + harmonicRatio. pitch via NCF (Normalized Cross-correlation Function) — default method per MATLAB R2025b. Defaults match MATLAB: WindowLength=round(0.052*fs)=832 samples for fs=16000, OverlapLength=round(0.042*fs)=672, hop=160, Range=[50,400] Hz. harmonicRatio uses 30/20ms windows (different defaults from pitch per MATLAB source). Frame counts (95 for pitch, 98 for harmonicRatio at 16001 samples) match MATLAB exactly. f0 estimates within ~1% of true frequency (220→220.45, 100→101.04 vs MATLAB 219.18, 100.63). harmonicRatio peaks at ~0.886 for pure sine (matches MATLAB to 4 decimals). Cycle F upgrade: harmonicRatio gained full MATLAB R2025b parity (auto low-edge from first sign change of R[k] + Smith's parabolic peak interpolation + clip [0,1]). Tiny ~2e-4 diff vs MATLAB on pure tones is FP-ordering noise. KNOWN GAPs: pitch methods PEF/CEP/LHS/SRH deferred (NCF only); pitchnn deferred (needs DNN runtime). Octave 11.1.0 doesn't ship pitch or harmonicRatio in core. Tolerance 5% for parity (algorithmic equivalence rather than bit-equality). |
