...
| `contains` | ✅ 🔬 | contains/startsWith/endsWith 'IgnoreCase' name-value vs MATLAB R2025b. DEEP-PROBE 2026-05-31: numkit parsed only (s,pat) and ignored the trailing 'IgnoreCase',<logical> pair — startsWith('Hello','he','IgnoreCase',true) returned false. Now a shared parseIgnoreCaseOption scans the trailing name-values; when set, both the source and each pattern are ASCII-lowercased before matching. startsWith('Hello','he','IgnoreCase',true)=1 (vs 0 without); endsWith('HELLO','lo',IC)=1; contains('HeLLo','ell',IC)=1; case-insensitive works with a cell-of-patterns too (contains('abc',{'XYZ','BC'},IC)=1). Logical results fingerprinted via double(). covers:[contains,startsWith,endsWith]. |
...
| `double` | ✅ | Sig: r = double(...). Type conversion. Spec-extension batch 2026-05-09. KNOWN GAP: numkit rejects double("string") with error; MATLAB returns NaN, Octave returns ASCII codes — both differ from numkit. String→double documented as separate gap; only int/logical/numeric paths pinned here. |
| `endsWith` | ✅ 🔬 | contains/startsWith/endsWith 'IgnoreCase' name-value vs MATLAB R2025b. DEEP-PROBE 2026-05-31: numkit parsed only (s,pat) and ignored the trailing 'IgnoreCase',<logical> pair — startsWith('Hello','he','IgnoreCase',true) returned false. Now a shared parseIgnoreCaseOption scans the trailing name-values; when set, both the source and each pattern are ASCII-lowercased before matching. startsWith('Hello','he','IgnoreCase',true)=1 (vs 0 without); endsWith('HELLO','lo',IC)=1; contains('HeLLo','ell',IC)=1; case-insensitive works with a cell-of-patterns too (contains('abc',{'XYZ','BC'},IC)=1). Logical results fingerprinted via double(). covers:[contains,startsWith,endsWith]. |
...
| `num2str` | ✅ 🔬 | Sig: r = num2str(X[, N|FMT]). DEFAULT precision is MAGNITUDE-AWARE (~4 digits after the integer part): num2str(1000000)="1000000", num2str(pi)="3.1416". DEEP-PROBE 2026-05-31: the default and integer-N forms were SCALAR-ONLY (threw 'Cannot convert double to scalar' on vectors/matrices); only the explicit FMT form handled arrays. Now the default/N forms synthesise MATLAB's column format and route through the FMT path: all-integer array -> "%<maxChars+2>.0f"; fractional -> "%<P+7>.<P>g" with P=max(floor(log10 maxabs),0)+5; the N form -> "%<N+7>.<N>g"; then common-blank-column strip. num2str([1 2 3])='1  2  3'; num2str([10 200 3])='10  200    3'; num2str([1.5 2.25 3])='1.5        2.25           3'; num2str([1 2;3 4]) is a 2x4 char matrix; negatives and the N form match. Cross-engine fingerprint via numel + sum(char codes). Complex ARRAY column layout still deferred. Spec-extension batch 2026-05-09. |
...
| `startsWith` | ✅ 🔬 | contains/startsWith/endsWith 'IgnoreCase' name-value vs MATLAB R2025b. DEEP-PROBE 2026-05-31: numkit parsed only (s,pat) and ignored the trailing 'IgnoreCase',<logical> pair — startsWith('Hello','he','IgnoreCase',true) returned false. Now a shared parseIgnoreCaseOption scans the trailing name-values; when set, both the source and each pattern are ASCII-lowercased before matching. startsWith('Hello','he','IgnoreCase',true)=1 (vs 0 without); endsWith('HELLO','lo',IC)=1; contains('HeLLo','ell',IC)=1; case-insensitive works with a cell-of-patterns too (contains('abc',{'XYZ','BC'},IC)=1). Logical results fingerprinted via double(). covers:[contains,startsWith,endsWith]. |
