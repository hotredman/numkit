...
| `geomean` | ✅ | 0.002 | 501.90× |  | OK | Sig g=geomean(X[,dim]): geometric mean = exp(mean(log x)). Vector geomean([1 2 3 4 5])=2.605171. Matrix default per-column: geomean(magic(4)) = [7.325683 6.814158 7.208434 5.943656]. dim=2 per-row: geomean(magic(4),2) = [5.943656 8.144476 ...]. |
...
| `harmmean` | ✅ | 0.002 | 161.62× |  | OK | Sig h=harmmean(X[,dim]): harmonic mean = n / sum(1./x). Vector harmmean([2 4 6 8])=3.84. Matrix default per-column: harmmean(magic(4)) cols (1,2)=(6.414254, 4.967742). dim=2 per-row pinned too. |
| `kurtosis` | ✅ | 0.002 | 354.19× |  | OK | Sig k=kurtosis(X[,flag[,dim]]): sample kurtosis (raw, ~3 for normal, NOT excess). Default flag=1 (biased): kurtosis([2 4 4 4 5 5 7 9])=2.78125. flag=0 (bias-corrected): 3.940625. Matrix per-column: kurtosis(magic(4)) cols (1,2)=(1.880697, 1.673068). |
| `mad` | ✅ | 0.003 | 692.84× |  | OK | Sig mad(x[,flag[,dim]]): mean (flag=0, default) or median (flag=1) absolute deviation. magic(4) per-column mean-AD = [4 4 ...]; median-AD = [2.5 3.5 ...]; dim=2 (per-row) mean-AD(1)=6. Vector mad([1 5 3 8 2 7 4 6]) = 2 (mean |x-4.5|). |
...
| `skewness` | ✅ | 0.004 | 200.15× |  | OK | Sig s=skewness(X[,flag[,dim]]): sample skewness. Default flag=1 (biased, /N): skewness([2 4 4 4 5 5 7 9])=0.65625. flag=0 (bias-corrected): 0.818488. Matrix is per-column: skewness(magic(4)) = [0.686021 -0.263374 0.263374 -0.686021] (antisymmetric over the magic columns). |
