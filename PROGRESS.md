...
| `groupsummary` | ✅ | Sig: [B, BG] = groupsummary(A, G, method). Array form, per-group reduction. A=[1..6], G=[1 1 2 2 3 3], method 'sum' -> B=[3;7;11], BG=[1;2;3]. (3rd output BC=counts is a numkit extension, covered in gtest, not fingerprinted.) Lifted to public C++ API numkit::builtin::groupsummary (GroupsummaryResult) 2026-06. |
...
| `max` | ✅ 🔬 | Sig: M = max(X). 1M-pt. 100 iters. Scalar fp. DEEP-PROBE 2026-05-29: max of an EMPTY array returns empty and never errors. Two-arg max(a,b) IGNORES NaN like MATLAB (b1=max(5,NaN)=5, b2=max(NaN,5)=5, bn=isnan(max(NaN,NaN))=1, va=max([NaN 6 8],[5 7 NaN])=[5 7 8]) — fixed 2026-06 (VM fast path used (a>=b)?a:b, Value-level used std::max; both -> fmax). |
...
| `min` | ✅ 🔬 | Sig: M = min(X). 1M-pt. 100 iters. Scalar fp. DEEP-PROBE 2026-05-29: min of an EMPTY array returns empty and never errors. Two-arg min(a,b) IGNORES NaN like MATLAB (b1=min(5,NaN)=5, b2=min(NaN,5)=5, bn=isnan(min(NaN,NaN))=1, va=min([NaN 6 8],[5 7 NaN])=[5 6 8]) — fixed 2026-06 (VM fast path + Value-level std::min -> fmin). |
