# builtin/mpower — ТЗ for completion
...
- Notes: Arithmetic-ops spec-extension batch (10 funcs). All bit-identical
  ...
  KNOWN GAP (separate ТЗ): mpower(matrix, n) matrix power not yet implemented in numkit — only scalar^scalar paths pinned. Would need O(log n) repeated mtimes for the matrix branch.
