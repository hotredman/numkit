...
- Notes: Gap #1 (cA/cD numeric values) closed by the wfilters
  Lo_D/Lo_R label-swap landed in 32ab3ce0 plus the dwt downsample-
  offset tweak in the same commit. dwt now bit-identical to MATLAB
  on db2, db4, sym4, coif1.

  Gap #2 (custom `(Lo_D, Hi_D)` form) closed in this commit. New
  `dwt_with_filters()` internal helper accepts the filter pair
  directly; adapter detects numeric 2nd arg and dispatches.

  Gap #3 (`'mode'` N-V) closed in this commit (parsed). Only 'sym'
  (the default) is supported — other modes throw a clear
  "not yet implemented" error rather than silently using 'sym'.

  Gap #4 (cD sign) was a transcription error in the audit — numkit
  returns positive endpoints just like MATLAB.

  Boundary modes 'per' / 'zpd' / 'ppd' / etc. DEFERRED.

  4 artefacts shipped (impl + 15-fp parity spec + 8 gtests in
  shared dwt_idwt_test.cpp + smoke). Bit-identical numkit ↔
  MATLAB. Octave doesn't ship `dwt`.
