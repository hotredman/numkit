...
- Notes: No source change needed. **Verified**: round-trip
  iswt(swt(x, 3, 'haar'), 'haar') recovers x to ~7e-16 machine
  precision (haar), ~1e-12 (db2). The detail-sign convention
  difference noted in closed/wavelet/swt.md is internally consistent
  — numkit's swt and iswt agree with each other, just on a different
  sign convention than MATLAB.
