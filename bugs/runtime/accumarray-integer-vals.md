...
- Fixed: 2026-06-05 (bug-fix loop, cycle 63),
  `toolboxes/builtin/src/language/arrays/accum.cpp`. `accumarray_reg` now promotes
...
- Live guard: `toolboxes/builtin/tests/accumarray_integer_vals_test.cpp` (6 TEST_F)
  ...
  `toolboxes/builtin/tests/smoke/accumarray_integer_vals_smoke.m`.
...
## Root cause
`toolboxes/builtin/src/language/arrays/accum.cpp` checked `vals.type() != DOUBLE`
...
## Guard
`toolboxes/builtin/tests/known_bugs_test.cpp` → `BuiltinKnownBug.AccumarrayIntegerVals`
(live) plus the dedicated `accumarray_integer_vals_test.cpp`.

## References
- `toolboxes/builtin/src/language/arrays/accum.cpp` (`accumarray_reg`, `toDoubleValue`,
