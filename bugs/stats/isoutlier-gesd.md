...
- Fixed: 2026-06-05 (bug-fix loop, cycle 24),
  `libs/stats/src/descriptive/descriptive_extras.cpp` (`gesdColumnMask`).
...
- Live guard: `libs/stats/tests/isoutlier_gesd_test.cpp` (7 TEST_F) + flipped
  ...
  `libs/stats/tests/smoke/isoutlier_gesd_smoke.m`.
- Note: `rmoutliers(x,'gesd')` and `filloutliers(...,'gesd')` remain script-only
  / deferred (separate adapters; out of scope here).

## References
- `libs/stats/src/descriptive/descriptive_extras.cpp` (isoutlier_reg, grubbs, gesdColumnMask)
