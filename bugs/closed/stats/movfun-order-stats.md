...
- Fixed: 2026-06-05 (bug-fix loop, cycle 50),
  `src/toolboxes/stats/src/moving/moving.cpp` (`movmin_impl`/`movmax_impl`/
...
- Live guard: `src/toolboxes/stats/tests/movfun_order_stats_test.cpp` (6 TEST_F) +
  `StatsKnownBug.MovfunOrderStats` flipped live. Parity:
  `tools/parity/specs/movfun_order_stats.json` (correctness=OK). Smoke:
  `src/toolboxes/stats/tests/smoke/movfun_order_stats_smoke.m`. This completes the mov*
...
## References
- `src/toolboxes/stats/src/moving/moving.cpp`
