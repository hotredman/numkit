...
- Fixed: 2026-06-05 (bug-fix loop, cycle 49),
  `src/toolboxes/stats/src/moving/moving.cpp`. The moving functions run through the
...
- Live guard: `src/toolboxes/stats/tests/movfun_typeclass_test.cpp` (5 TEST_F) +
  `StatsKnownBug.MovfunTypeClass` flipped live. Parity:
  `tools/parity/specs/movfun_typeclass.json` (correctness=OK). Smoke:
  `src/toolboxes/stats/tests/smoke/movfun_typeclass_smoke.m`.
...
## References
- `src/toolboxes/stats/src/moving/moving.cpp` (`movmean_impl`/`movsum_impl`/
