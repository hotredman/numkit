...
- Fixed: 2026-06-05 (bug-fix loop, cycle 63),
  `libs/builtin/src/language/arrays/accum.cpp`. `accumarray_reg` now promotes
...
- Live guard: `libs/builtin/tests/accumarray_integer_vals_test.cpp` (6 TEST_F)
  ...
  `libs/builtin/tests/smoke/accumarray_integer_vals_smoke.m`.
...
## Root cause
`libs/builtin/src/language/arrays/accum.cpp` checked `vals.type() != DOUBLE`
...
## Guard
`libs/builtin/tests/known_bugs_test.cpp` → `BuiltinKnownBug.AccumarrayIntegerVals`
(live) plus the dedicated `accumarray_integer_vals_test.cpp`.

## References
- `libs/builtin/src/language/arrays/accum.cpp` (`accumarray_reg`, `toDoubleValue`,
