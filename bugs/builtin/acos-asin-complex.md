# builtin.acos / asin — return NaN for |x|>1 instead of a complex value
...
## References
- `libs/builtin/src/math/trig/trig_{highway,portable}.cpp` (acos/asin)
...
- Added an out-of-`[-1,1]` check in both the Highway and portable acos/asin:
  ...
  and NaN stay real.
- Live guard: `libs/builtin/tests/acos_asin_complex_test.cpp` (4 cases).
  Parity: `tools/parity/specs/{acos,asin}.json` extended (correctness=OK).
  Smoke: `libs/builtin/tests/smoke/acos_asin_complex_smoke.m`.
- Spin-off finding: `sqrt`/`acosh`/`atanh` have the same *array* gap (promote
  only scalars) — catalogued as bugs/builtin/complex-promotion-arrays.md.
