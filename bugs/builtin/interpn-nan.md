...
## Where
`libs/builtin/src/math/interp/interp.cpp` `interpn_reg` (registered in
`libs/builtin/src/library.cpp`). The ndim-dispatch likely falls through to a
...
## Guard
`libs/builtin/tests/known_bugs_test.cpp` → `BuiltinKnownBug.DISABLED_InterpnOneDimNaN`
