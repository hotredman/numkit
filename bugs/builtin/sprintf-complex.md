# builtin.sprintf/fprintf — throw on complex arguments
...
- Fixed: 2026-06-05 (bug-fix loop, cycle 51),
  `libs/builtin/src/language/strings/format.cpp` (`formatCyclic`). The format
...
- Live guard: `libs/builtin/tests/sprintf_complex_test.cpp` (5 TEST_F) +
  `BuiltinKnownBug.SprintfComplex` flipped live. Parity:
  `tools/parity/specs/sprintf_complex.json` (correctness=OK). Smoke:
  `libs/builtin/tests/smoke/sprintf_complex_smoke.m`.
...
## References
- `libs/builtin/src/language/strings/format.cpp` (`formatCyclic`)
