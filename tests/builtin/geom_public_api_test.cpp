...
TEST(GeomPublicApiTest, Polyarea)
{
    ...
    // unit square -> area 1
    EXPECT_DOUBLE_EQ(numkit::math::polyarea(x, y, e.resource()).toScalar(), 1.0);
    // mr defaults to nullptr -> process default resource
    EXPECT_DOUBLE_EQ(numkit::math::polyarea(x, y).toScalar(), 1.0);
    // triangle (0,0)-(1,0)-(0,1) -> area 0.5
    Value xt = var(e, "[0 1 0]", "xt");
    Value yt = var(e, "[0 0 1]", "yt");
    EXPECT_DOUBLE_EQ(numkit::math::polyarea(xt, yt, e.resource()).toScalar(), 0.5);
    // < 3 vertices -> 0
    Value x2 = var(e, "[0 1]", "x2");
    Value y2 = var(e, "[0 1]", "y2");
    EXPECT_DOUBLE_EQ(numkit::math::polyarea(x2, y2, e.resource()).toScalar(), 0.0);
    // shape mismatch throws
    Value yb = var(e, "[0 0 1]", "yb");
    EXPECT_ANY_THROW(numkit::math::polyarea(x, yb, e.resource()));
}
...
TEST(GeomPublicApiTest, Inpolygon)
{
    ...
    Value in = numkit::math::inpolygon(xq, yq, xv, yv, e.resource());
    ...
    Value deg = numkit::math::inpolygon(xq, yq, var(e, "[0 1]", "xv2"),
                                   var(e, "[0 1]", "yv2"), e.resource());
    EXPECT_EQ(deg.logicalData()[0], 0);
    // query shape mismatch throws
    EXPECT_ANY_THROW(numkit::math::inpolygon(xq, var(e, "[0 0]", "yqb"), xv, yv, e.resource()));
}
...
TEST(GeomPublicApiTest, Convhull)
{
    ...
    Value y = var(e, "[0 0 1 1]", "y");
    Value K = numkit::math::convhull(x, y, e.resource());
    ...
    Value K2 = numkit::math::convhull(xi, yi, e.resource());
    EXPECT_EQ(K2.numel(), 5u);
    for (std::size_t i = 0; i < K2.numel(); ++i)
        EXPECT_NE(K2.doubleData()[i], 5.0);
    // < 3 points -> [1; 2; 1]
    Value K3 = numkit::math::convhull(var(e, "[0 1]", "x3"), var(e, "[0 1]", "y3"), e.resource());
    EXPECT_EQ(K3.numel(), 3u);
    // shape mismatch throws
    EXPECT_ANY_THROW(numkit::math::convhull(x, var(e, "[0 0 1]", "yb2"), e.resource()));
}
...
TEST(GeomPublicApiTest, Delaunay)
{
    ...
    Value T1 = numkit::math::delaunay(var(e, "[0 1 0]", "xt"), var(e, "[0 0 1]", "yt"),
                                 e.resource());
    ...
    Value T = numkit::math::delaunay(x, y, e.resource());
    ...
    // < 3 points -> 0x3
    Value T0 = numkit::math::delaunay(var(e, "[0 1]", "x2"), var(e, "[0 1]", "y2"),
                                 e.resource());
    EXPECT_EQ(T0.dims().rows(), 0u);
    EXPECT_EQ(T0.dims().cols(), 3u);
    // shape mismatch throws
    EXPECT_ANY_THROW(numkit::math::delaunay(x, var(e, "[0 0 1]", "yb4"), e.resource()));
}
...
TEST(GeomPublicApiTest, Griddata)
{
    ...
    Value vq = numkit::math::griddata(x, y, v, xq, yq, e.resource());
    ...
    // < 3 samples -> all NaN
    Value vq0 = numkit::math::griddata(var(e, "[0 1]", "x2"), var(e, "[0 1]", "y2"),
                                  var(e, "[1 2]", "v2"), xq, yq, e.resource());
    EXPECT_TRUE(std::isnan(vq0.doubleData()[0]));
    // x/y/v numel mismatch throws
    EXPECT_ANY_THROW(
        numkit::math::griddata(x, y, var(e, "[1 2 3]", "vb"), xq, yq, e.resource()));
    // xq/yq numel mismatch throws
    EXPECT_ANY_THROW(
        numkit::math::griddata(x, y, v, xq, var(e, "[0 0]", "yqb"), e.resource()));
}
...
TEST(GeomPublicApiTest, Boundary)
{
    ...
    Value b0 = numkit::math::boundary(x, y, 0.0, e.resource());
    Value k = numkit::math::convhull(x, y, e.resource());
    EXPECT_EQ(b0.numel(), k.numel());
    // shrink > 0 -> closed polygon (first index repeated at the end)
    Value b = numkit::math::boundary(x, y, 1.0, e.resource());
    ASSERT_GE(b.numel(), 2u);
    EXPECT_DOUBLE_EQ(b.doubleData()[0], b.doubleData()[b.numel() - 1]);
    // shape mismatch throws
    EXPECT_ANY_THROW(numkit::math::boundary(x, var(e, "[0 0 1]", "yb3"), 0.5, e.resource()));
}
...
TEST(GeomPublicApiTest, Histcounts2)
{
    ...
    Value N = numkit::math::histcounts2(x, y, xe, ye);
    ...
    Value N2 = numkit::math::histcounts2(var(e, "[5 0.5]", "x2"),
