...
test.describe('Figures pane', () => {
  ...
  test.beforeEach(async () => {
    ...
    // Seed a single figure. `figure(); plot(...)` is the simplest
    // way; the engine emits __FIGURE_DATA__ which the renderer turns
    // into one card.
    await ide.runScript(
      'import compat.*;\n'
      + 'figure;\n'
  ...
  test('clicking the card body opens the figure window', async () => {
    await ide.figureCards.first().locator('.fp-card-body').click();
    await expect(ide.figureWindow).toBeVisible({ timeout: 5_000 });
  });

  test('clicking the card title bar (not on a button) opens the figure window', async () => {
  ...
  test('× button removes the card and does not open the figure window', async () => {
  ...
  test('redundant expand-icon button was removed (only × in title bar)', async () => {
  ...
  test('Close all clears every figure card', async () => {
