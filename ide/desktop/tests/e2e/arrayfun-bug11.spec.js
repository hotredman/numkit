...
test.describe('arrayfun — BUG #11', () => {
  test('arrayfun(@(x) x*2, 1:5) → [2 4 6 8 10] (not [1..5])', async ({ ide, page }) => {
    await ide.runScript(
      'import compat.*;\n'
  ...
  test('arrayfun(@(x) x^2, 1:4) → [1 4 9 16]', async ({ ide, page }) => {
    await ide.runScript(
      'import compat.*;\n'
  ...
  test('arrayfun with multiple input arrays', async ({ ide, page }) => {
    await ide.runScript(
      'import compat.*;\n'
  ...
  test('arrayfun + UniformOutput=false → cell array', async ({ ide, page }) => {
    await ide.runScript(
      'import compat.*;\n'
