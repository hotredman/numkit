...
Value strel(const std::string &shape, const std::vector<double> &params, const Value &arbitrary_nhood, std::pmr::memory_resource *mr = nullptr);
...
Value imerode(const Value &I, const Value &SE, std::pmr::memory_resource *mr = nullptr);

/// imdilate(I, SE) — grayscale dilation (output = max within SE).
Value imdilate(const Value &I, const Value &SE, std::pmr::memory_resource *mr = nullptr);

/// imopen(I, SE)  = imdilate(imerode(I, SE), SE).
Value imopen(const Value &I, const Value &SE, std::pmr::memory_resource *mr = nullptr);

/// imclose(I, SE) = imerode(imdilate(I, SE), SE).
Value imclose(const Value &I, const Value &SE, std::pmr::memory_resource *mr = nullptr);
...
Value imreconstruct(const Value &marker, const Value &mask, int conn, std::pmr::memory_resource *mr = nullptr);
...
Value imfill_holes(const Value &BW, int conn, std::pmr::memory_resource *mr = nullptr);
...
Value imregionalmax(const Value &I, int conn, std::pmr::memory_resource *mr = nullptr);
...
Value imregionalmin(const Value &I, int conn, std::pmr::memory_resource *mr = nullptr);
...
Value imhmax(const Value &I, double h, int conn, std::pmr::memory_resource *mr = nullptr);

/// `J = imhmin(I, h [, conn])` — h-minima transform; dual of imhmax
/// via image inversion.
Value imhmin(const Value &I, double h, int conn, std::pmr::memory_resource *mr = nullptr);
...
Value imextendedmax(const Value &I, double h, int conn, std::pmr::memory_resource *mr = nullptr);
...
Value imextendedmin(const Value &I, double h, int conn, std::pmr::memory_resource *mr = nullptr);
...
Value imimposemin(const Value &I, const Value &BW, int conn, std::pmr::memory_resource *mr = nullptr);
...
Value imclearborder(const Value &BW, int conn, std::pmr::memory_resource *mr = nullptr);
...
Value imkeepborder(const Value &BW, int conn, std::pmr::memory_resource *mr = nullptr);
...
Value imtophat(const Value &I, const Value &SE, std::pmr::memory_resource *mr = nullptr);

/// `J = imbothat(I, SE)` — black top-hat: imclose(I, SE) − I.
/// Highlights dark structures smaller than SE on a bright
/// background. Output saturates to the input class.
Value imbothat(const Value &I, const Value &SE, std::pmr::memory_resource *mr = nullptr);
...
Value mmgradm(const Value &I, const Value &se_dil, const Value &se_ero, std::pmr::memory_resource *mr = nullptr);
...
Value bwunpack(const Value &BWP, size_t M, std::pmr::memory_resource *mr = nullptr);
...
Value applylut(const Value &BW, const Value &LUT, std::pmr::memory_resource *mr = nullptr);
...
Value bwhitmiss(const Value &BW, const Value &se1, const Value &se2, std::pmr::memory_resource *mr = nullptr);
...
Value bwmorph(const Value &BW, const std::string &op, int n, std::pmr::memory_resource *mr = nullptr);
