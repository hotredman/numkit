...
Value padarray(const Value &x, const std::vector<int> &padsize, PadMode mode, double pad_value, const std::string &direction, std::pmr::memory_resource *mr = nullptr);

/// fspecial(type, ...) — returns a 2-D filter kernel as a DOUBLE matrix.
/// Supported types: 'average', 'gaussian', 'laplacian', 'log',
/// 'sobel', 'prewitt', 'disk'.
Value fspecial(const std::string &type, const std::vector<double> &params, std::pmr::memory_resource *mr = nullptr);
...
Value imfilter(const Value &I, const Value &h, PadMode boundary, double pad_value, bool full, bool flip_kernel, std::pmr::memory_resource *mr = nullptr);

/// imgaussfilt(I, sigma[, FilterSize]) — 2-D Gaussian filtering with
/// boundary='replicate', output='same'.
Value imgaussfilt(const Value &I, double sigma, int filter_size, std::pmr::memory_resource *mr = nullptr);

/// imboxfilt(I, FilterSize) — local mean filter with replicate boundary.
Value imboxfilt(const Value &I, int filter_size, std::pmr::memory_resource *mr = nullptr);

/// medfilt2(I[, [m n]]) — 2-D median filter. Default 3×3.
Value medfilt2(const Value &I, int rows = 3, int cols = 3, std::pmr::memory_resource *mr = nullptr);
...
Value imsharpen(const Value &I, double radius, double amount, double threshold, std::pmr::memory_resource *mr = nullptr);
...
Value im2col(const Value &A, int m, int n, const std::string &block_type, std::pmr::memory_resource *mr = nullptr);
...
Value col2im(const Value &B, int m, int n, int mm, int nn, const std::string &block_type, std::pmr::memory_resource *mr = nullptr);
...
Value imbilatfilt(const Value &I, double degreeOfSmoothing, double spatialSigma, std::pmr::memory_resource *mr = nullptr);
...
Value imnoise(const Value &I, const std::string &mode, const Value &p1, const Value &p2, std::pmr::memory_resource *mr = nullptr);
...
Value stdfilt(const Value &I, const Value &domain, std::pmr::memory_resource *mr = nullptr);
...
Value rangefilt(const Value &I, const Value &domain, std::pmr::memory_resource *mr = nullptr);
...
Value imsmooth(const Value &I, const std::string &name, double sigma, std::pmr::memory_resource *mr = nullptr);
...
Value imboxfilt3(const Value &V, int fH, int fW, int fP, std::pmr::memory_resource *mr = nullptr);
...
Value convmtx2(const Value &h, int m, int n, std::pmr::memory_resource *mr = nullptr);
...
Value imgaussfilt3(const Value &V, double sigH, double sigW, double sigP, std::pmr::memory_resource *mr = nullptr);
...
Value medfilt3(const Value &V, int M, int N, int P, std::pmr::memory_resource *mr = nullptr);
...
Value entropyfilt(const Value &I, const Value &domain, std::pmr::memory_resource *mr = nullptr);
...
Value ordfilt2(const Value &A, int nth, const Value &domain, const Value &S, PadMode boundary, double pad_value, std::pmr::memory_resource *mr = nullptr);
...
freqz2(const Value &h, size_t M, size_t N, std::pmr::memory_resource *mr = nullptr);
...
wiener2(const Value &I, size_t nh, size_t nw, double noise, std::pmr::memory_resource *mr = nullptr);
