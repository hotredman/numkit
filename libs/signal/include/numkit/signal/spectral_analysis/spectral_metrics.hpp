...
Value bandpower(const Value &x, const Value &fs = Value::Empty, const Value &freqrange = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// meanfreq(x[, fs]) — power-weighted mean frequency:
/// sum(f * P) / sum(P).
Value meanfreq(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// medfreq(x[, fs]) — median frequency: F such that the cumulative
/// power below F equals half the total power.
Value medfreq(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// enbw(window[, fs]) — equivalent noise bandwidth of a window:
/// fs * sum(w.^2) / sum(w).^2  (returns dimensionless if fs absent).
Value enbw(const Value &window, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// obw(x[, fs[, p]]) — occupied-bandwidth at fraction p of total power
/// (default p = 0.99). Returns the bandwidth (Hz or rad/s).
Value obw(const Value &x, const Value &fs = Value::Empty, double p = 0.99, std::pmr::memory_resource *mr = nullptr);

/// powerbw(x[, fs]) — 3-dB (half-peak) power bandwidth around the
/// dominant peak.
Value powerbw(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// spectralcrest(x[, fs]) — max(P) / mean(P). Peakedness ratio.
Value spectralcrest(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// spectralflatness(x[, fs]) — geomean(P) / mean(P). 1 = white, 0 = tonal.
Value spectralflatness(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// spectralentropy(x[, fs]) — Shannon entropy of the normalised PSD.
Value spectralentropy(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// spectralkurtosis(x[, fs]) — kurtosis of the PSD, treated as a
/// distribution over frequency.
Value spectralkurtosis(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// spectralskewness(x[, fs]) — skewness of the PSD as a frequency distribution.
Value spectralskewness(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);
...
Value snr(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// sinad(x[, fs]) — signal-to-noise-and-distortion in dB. Like snr
/// but treats harmonics as noise (denominator includes them).
Value sinad(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// thd(x[, fs]) — total harmonic distortion in dB. Power sum of the
/// first 6 harmonics over the fundamental.
Value thd(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);
...
Value sfdr(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);
...
Value instfreq(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);

/// instbw(x[, fs]) — instantaneous bandwidth: |dA/dt| / (2π·A) where
/// A is the analytic-signal envelope. Column vector of length
/// numel(x)-1.
Value instbw(const Value &x, const Value &fs = Value::Empty, std::pmr::memory_resource *mr = nullptr);
