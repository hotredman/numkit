...
/// to length N first.
Value cconv(const Value &x, const Value &y, size_t n = 0, std::pmr::memory_resource *mr = nullptr);

/// Convolution matrix: convmtx(h, n) returns an (n+len(h)-1)×n matrix
/// such that A * x == conv(h, x) for any column vector x of length n.
Value convmtx(const Value &h, size_t n, std::pmr::memory_resource *mr = nullptr);

/// 2-D cross-correlation: xcorr2(A, B) returns a (rA+rB-1)×(cA+cB-1)
/// real matrix C[k,l] = sum_{i,j} A[i,j] * B[i-k+rA-1, j-l+cA-1].
Value xcorr2(const Value &A, const Value &B, std::pmr::memory_resource *mr = nullptr);
...
long finddelay(const Value &x, const Value &y, long max_lag = 0, std::pmr::memory_resource *mr = nullptr);

/// Align x and y by zero-padding the leading signal. Returns
/// (xa, ya) — both same length, with the lag from finddelay applied.
std::tuple<Value, Value>
alignsignals(const Value &x, const Value &y, long max_lag = 0, std::pmr::memory_resource *mr = nullptr);
