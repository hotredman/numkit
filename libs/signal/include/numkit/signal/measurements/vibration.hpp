...
envspectrum(std::pmr::memory_resource *mr, const Value &x, double fs = 0.0);
...
tachorpm(std::pmr::memory_resource *mr, const Value &x, double fs,
         const Value *threshold = nullptr, int ppr = 1);
...
tsa(std::pmr::memory_resource *mr, const Value &x, double fs,
    const Value &rpm, double fs_rpm, int n_per_rev = 1024);
