...
/// @return    Tuple `(peakValues, peakIndices)`. Indices are 1-based to
///            match MATLAB; both outputs are row vectors of the same
///            length, empty when no peaks exist.
///
/// @note Plateaus (`x[i] == x[i+1]`) do not count as peaks under the
///       strict-inequality rule. MATLAB's `findpeaks` with default
///       options also uses strict inequality.
///
/// @code
