...
//   3. rebuild monic poly p      (builtin::poly)
...
Value polystab(const Value &a, std::pmr::memory_resource *mr)
{
    ...
    // ── Step 1: roots of a ───────────────────────────────────────────
    ScratchArena arena(mr);
    Value rv = builtin::roots(a, &arena);
    ...
    // builtin::poly needs a COMPLEX vector to yield a COMPLEX result;
    ...
    Value monic = builtin::poly(rootVec, &arena);
