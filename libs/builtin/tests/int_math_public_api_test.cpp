// libs/builtin/tests/int_math_public_api_test.cpp
//
...
TEST(IntMathPublicApi, Bitget)
{
    ...
    EXPECT_DOUBLE_EQ(
        builtin::bitget(a, bvar(e, "1", "n1"), e.resource()).toScalar(), 0.0);
    EXPECT_DOUBLE_EQ(
        builtin::bitget(a, bvar(e, "4", "n4"), e.resource()).toScalar(), 1.0);
    // vector bit positions broadcast against the scalar a
    Value bits = builtin::bitget(a, bvar(e, "[1 2 3 4]", "nv"), e.resource());
...
TEST(IntMathPublicApi, Bitset)
{
    ...
    EXPECT_DOUBLE_EQ(builtin::bitset(bvar(e, "1", "o"), bvar(e, "3", "n3b"),
                                     Value::Empty, e.resource())
                         .toScalar(),
                     5.0);
    // val = 0 clears the bit: bitset(7,1,0) = 6
    EXPECT_DOUBLE_EQ(builtin::bitset(bvar(e, "7", "s"), bvar(e, "1", "n1"),
                                     bvar(e, "0", "v0"), e.resource())
                         .toScalar(),
                     6.0);
    // val = 1 sets the bit: bitset(7,4,1) = 15
    EXPECT_DOUBLE_EQ(builtin::bitset(bvar(e, "7", "s2"), bvar(e, "4", "n4b"),
                                     bvar(e, "1", "v1"), e.resource())
                         .toScalar(),
                     15.0);
    // val outside {0,1} throws
    EXPECT_ANY_THROW(builtin::bitset(bvar(e, "1", "o2"), bvar(e, "2", "n2b"),
