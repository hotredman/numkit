// libs/builtin/tests/permutations_public_api_test.cpp
//
...
TEST(PermutationsPublicApi, Colperm)
{
    ...
    Value p2 =
        builtin::colperm(pv(e, "[-1 0 0; 2 -3 0; 0 4 5]", "S2"), e.resource());
...
TEST(PermutationsPublicApi, Symrcm)
{
    ...
    Value pi = builtin::symrcm(pv(e, "eye(3)", "I"), e.resource());
    ASSERT_EQ(pi.numel(), 3u);
    EXPECT_DOUBLE_EQ(pi.doubleData()[0], 1.0);
    EXPECT_DOUBLE_EQ(pi.doubleData()[2], 3.0);
    // non-square throws
    EXPECT_ANY_THROW(builtin::symrcm(pv(e, "[1 2 3; 4 5 6]", "R"), e.resource()));
}
