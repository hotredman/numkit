// libs/builtin/tests/discrete_setops_public_api_test.cpp
//
...
TEST(DiscreteSetopsPublicApi, SetxorAllNumUnique)
{
    ...
    // allunique
    EXPECT_NE(builtin::allunique(sv(e, "[1 2 3]", "u1"), e.resource()).toScalar(),
              0.0);
    EXPECT_EQ(builtin::allunique(sv(e, "[1 2 2]", "u2"), e.resource()).toScalar(),
              0.0);
    // numunique
    EXPECT_DOUBLE_EQ(
        builtin::numunique(sv(e, "[1 2 2 3 3 3]", "n"), e.resource()).toScalar(),
        3.0);
}
...
TEST(DiscreteSetopsPublicApi, IsmemberUniqueTol)
{
    ...
    Value tf2 = builtin::ismembertol(sv(e, "[2]", "q2"), sv(e, "[1 2 3]", "s2"),
                                     1e-6, e.resource());
    EXPECT_NE(tf2.elemAsDouble(0), 0.0);
    // uniquetol collapses exact duplicates -> [1 2 3]
    Value u = builtin::uniquetol(sv(e, "[1 2 2 3]", "x"), 1e-6, e.resource());
