...
TEST(PermutationsPublicApi, Colperm)
{
    ...
    Value p =
        numkit::math::colperm(pv(e, "[0 1 0 1; 1 1 1 0; 0 1 0 0; 1 0 0 0]", "S"));
    ...
    Value p2 =
        numkit::math::colperm(pv(e, "[-1 0 0; 2 -3 0; 0 4 5]", "S2"), e.resource());
...
TEST(PermutationsPublicApi, Symrcm)
{
    ...
    Value pi = numkit::math::symrcm(pv(e, "eye(3)", "I"), e.resource());
    ASSERT_EQ(pi.numel(), 3u);
    EXPECT_DOUBLE_EQ(pi.doubleData()[0], 1.0);
    EXPECT_DOUBLE_EQ(pi.doubleData()[2], 3.0);
    // non-square throws
    EXPECT_ANY_THROW(numkit::math::symrcm(pv(e, "[1 2 3; 4 5 6]", "R"), e.resource()));
}
