...
TEST(SimdParity_Sin, WithinUlpBudget)
{
    checkTranscendentalParity(
        [](std::pmr::memory_resource *a, const Value &x) { return numkit::math::sin(x, a); },
...
{
    checkTranscendentalParity(
        [](std::pmr::memory_resource *a, const Value &x) { return numkit::math::cos(x, a); },
...
TEST(SimdParity_Exp, WithinUlpBudget)
{
    ...
    checkTranscendentalParity(
        [](std::pmr::memory_resource *a, const Value &x) { return numkit::math::exp(x, a); },
...
TEST(SimdParity_Log, WithinUlpBudget)
{
    ...
    checkTranscendentalParity(
        [](std::pmr::memory_resource *a, const Value &x) { return numkit::math::log(x, a); },
...
{
    checkTranscendentalParity(
        [](std::pmr::memory_resource *a, const Value &x) { return numkit::math::expm1(x, a); },
...
TEST(SimdParity_Log1p, WithinUlpBudget)
{
    ...
    checkTranscendentalParity(
        [](std::pmr::memory_resource *a, const Value &x) { return numkit::math::log1p(x, a); },
...
TEST(SimdParity_Log2, WithinUlpBudget)
{
    ...
    checkTranscendentalParity(
        [](std::pmr::memory_resource *a, const Value &x) { return numkit::math::log2(x, a); },
...
TEST(SimdParity_Erf, WithinUlpBudget)
{
    ...
    checkTranscendentalParity(
        [](std::pmr::memory_resource *a, const Value &x) { return numkit::math::erf(x, a); },
...
TEST(SimdParity_Transcendental, NegativeLogScalarStillComplex)
{
    ...
    Value y = numkit::math::log(Value::scalar(-1.0, mr), mr);
...
{
    checkBinaryParity(
        [](std::pmr::memory_resource *a, const Value &x, const Value &y) { return numkit::lang::plus(x, y, a); },
...
{
    checkBinaryParity(
        [](std::pmr::memory_resource *a, const Value &x, const Value &y) { return numkit::lang::minus(x, y, a); },
...
{
    checkBinaryParity(
        [](std::pmr::memory_resource *a, const Value &x, const Value &y) { return numkit::lang::times(x, y, a); },
...
{
    checkBinaryParity(
        [](std::pmr::memory_resource *a, const Value &x, const Value &y) { return numkit::lang::rdivide(x, y, a); },
...
TEST(SimdParity_Mtimes, MatchesScalarSquareMatrix)
{
    ...
    Value C = numkit::lang::mtimes(A, B, mr);
...
TEST(SimdParity_Dim, PlusOn1DRowAndColumn)
{
    ...
    auto cRow = numkit::lang::plus(aRow, bRow, mr);
    ...
    auto cCol = numkit::lang::plus(aCol, bCol, mr);
...
TEST(SimdParity_Mtimes, ThrowsOn3DInput)
{
    ...
    // quietly used (rows, cols) and ignored pages.
    {
        ...
        EXPECT_THROW({ (void)numkit::lang::mtimes(A, B, mr); },
                     numkit::Error);
    }
...
TEST(SimdParity_Mtimes, ScalarTimes3DArrayStillWorks)
{
    ...
    auto C = numkit::lang::mtimes(s, A, mr);
...
TEST(SimdParity_ParallelLarge, PlusBitIdenticalAcrossSplits)
{
    ...
    Value z = numkit::lang::plus(x, y, mr);
    ASSERT_EQ(z.numel(), N);
    for (size_t i = 0; i < N; ++i)
...
TEST(SimdParity_ParallelLarge, MinusBitIdenticalAcrossSplits)
{
    ...
    Value x = makeDoubleVector(mr, a);
    Value y = makeDoubleVector(mr, b);
    Value z = numkit::lang::minus(x, y, mr);
    ASSERT_EQ(z.numel(), N);
    for (size_t i = 0; i < N; ++i)
        EXPECT_TRUE(bitEquals(z.doubleData()[i], a[i] - b[i])) << "i=" << i;
}
...
TEST(SimdParity_ParallelLarge, TimesBitIdenticalAcrossSplits)
{
    ...
    Value x = makeDoubleVector(mr, a);
    Value y = makeDoubleVector(mr, b);
    Value z = numkit::lang::times(x, y, mr);
    ASSERT_EQ(z.numel(), N);
    for (size_t i = 0; i < N; ++i)
        EXPECT_TRUE(bitEquals(z.doubleData()[i], a[i] * b[i])) << "i=" << i;
}
...
TEST(SimdParity_ParallelLarge, RdivideBitIdenticalAcrossSplits)
{
    ...
    Value x = makeDoubleVector(mr, a);
    Value y = makeDoubleVector(mr, b);
    Value z = numkit::lang::rdivide(x, y, mr);
    ASSERT_EQ(z.numel(), N);
    for (size_t i = 0; i < N; ++i)
        EXPECT_TRUE(bitEquals(z.doubleData()[i], a[i] / b[i])) << "i=" << i;
}
...
TEST(SimdParity_ParallelLarge, SinDeterministicAcrossCalls)
{
    ...
    Value x  = makeDoubleVector(mr, src);
    Value y1 = numkit::math::sin(x, mr);
    Value y2 = numkit::math::sin(x, mr);
    Value y3 = numkit::math::sin(x, mr);
...
TEST(SimdParity_ParallelLarge, ExpDeterministicAcrossCalls)
{
    ...
    Value x  = makeDoubleVector(mr, src);
    Value y1 = numkit::math::exp(x, mr);
    Value y2 = numkit::math::exp(x, mr);
    ASSERT_EQ(y1.numel(), N);
    for (size_t i = 0; i < N; ++i)
        EXPECT_TRUE(bitEquals(y1.doubleData()[i], y2.doubleData()[i])) << "i=" << i;
}
