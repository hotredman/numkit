// toolboxes/control/src/discretize/discretize.cpp
//
...
// toolboxes/control/internal/numerics — same pair the cycle-34 step
...
// (CallContext wrappers) live in discretize_reg.cpp.
...
Value packResult(const Mat &Ad, const Vec &Bd, const Vec &Cd, double Dd, size_t n, double Ts, const std::string &origKind, std::pmr::memory_resource *mr)
{
    ...
    // Convert (A, B, C, D) → (num, den) via toolboxes/control's ss2tf.
