...
// G3: a bridged CHAR-array result (num2str) -- compiles in BRIDGED mode, links nk_codegen_rt,
// unboxes the char array via bridge_to_vec_char (new C-ABI nk_unbox_char_array), RUNS, and
// matches the interpreter (the bridge calls the same runtime num2str, so the format is the
// interpreter's BY CONSTRUCTION -- no C++ format-replication, no divergence).
TEST(CodegenBridge, BridgedNum2strCharArrayRunsAndMatchesInterpreter)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const EmittedFunction emitted =
        emitFunction(*fn, {{"x", InferredType::scalar(ValueType::DOUBLE)}}, reg, nullptr, bridge);
    ASSERT_NE(emitted.source.find("nk_rt::bridge_to_vec_char(\"num2str\""), std::string::npos)
        << "a CHAR-array bridged result must unbox via bridge_to_vec_char";

    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_bridged_num2str_e2e.exe").string();
    const std::string outTxt = (base / "nk_bridged_num2str_e2e_out.txt").string();
    std::error_code   ec;
    std::filesystem::remove(outTxt, ec);
    ...
    program +=
        ...
        "  std::FILE* g = std::fopen(\"" + fwd(outTxt) + "\", \"w\");\n"
        "  if (!g) return 2;\n"
        "  std::fprintf(g, \"%.17g\\n\", f(42.0));\n"  // num2str(42)='42': '4'(52) + 2*1000 = 2052
        "  std::fclose(g); return 0;\n}\n";
    ...
    ASSERT_EQ(got.size(), 1u);
    numkit::StandardEngine engine;
    const double interp = engine.eval("s = num2str(42); s(1) + numel(s) * 1000", true).toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
    EXPECT_DOUBLE_EQ(got[0], 2052.0);  // num2str(42)='42': '4'=52, numel=2 -> 52 + 2000
}

// G3b: sprintf via bridge -- the CHAR format LITERAL is boxed (nk_box_string), value args boxed
// as before, the runtime formats, and the CHAR result unboxes via bridge_to_vec_char. The
// interpreter owns printf-style formatting -> exact by construction.
TEST(CodegenBridge, BridgedSprintfRunsAndMatchesInterpreter)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const EmittedFunction emitted =
        emitFunction(*fn, {{"x", InferredType::scalar(ValueType::DOUBLE)}}, reg, nullptr, bridge);
    ASSERT_NE(emitted.source.find("nk_box_string(\"%d!\")"), std::string::npos)
        << "the sprintf format literal must box via nk_box_string";
    ASSERT_NE(emitted.source.find("nk_rt::bridge_to_vec_char(\"sprintf\""), std::string::npos);

    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_bridged_sprintf_e2e.exe").string();
    const std::string outTxt = (base / "nk_bridged_sprintf_e2e_out.txt").string();
    std::error_code   ec;
    std::filesystem::remove(outTxt, ec);
    ...
    numkit::StandardEngine engine;
    const double interp =
        engine.eval("s = sprintf('%d!', 42); s(1) + numel(s) * 1000", true).toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
    EXPECT_DOUBLE_EQ(got[0], 3052.0);  // '42!': '4'=52, numel=3 -> 52 + 3000
}

// G4a: a cell literal {a,b,c} + content extraction c{i} as Dynamic values. The
// brace literal builds a Dynamic cell (nk_box_cell; each element boxed by the
// Dynamic tier, reusing it whole), c{i} extracts content (nk_cell_get), and the
// Dynamic chain r = c{1}+c{2}+c{3} returns via the boxed-nk_val ABI (A3.5).
// Compiles, links the runtime DLL, runs, and diffs against the interpreter.
TEST(CodegenBridge, BridgedCellLiteralAndContentGetRunsAndMatchesInterpreter)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const EmittedFunction emitted =
        emitFunction(*fn, {{"x", InferredType::scalar(ValueType::DOUBLE)}}, reg, nullptr, bridge);
    ASSERT_NE(emitted.source.find("nk_rt::val::cell({"), std::string::npos)
        << "the brace literal must build a Dynamic cell";
    ASSERT_NE(emitted.source.find("nk_rt::cell_get("), std::string::npos)
        << "c{i} must extract content via the runtime";

    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_cell_e2e.exe").string();
    const std::string outTxt = (base / "nk_cell_e2e_out.txt").string();
    std::error_code   ec;
    std::filesystem::remove(outTxt, ec);
    ...
    ASSERT_EQ(got.size(), 1u);
    numkit::StandardEngine engine;
    const double interp = engine.eval("c = {5, 6, 50}; c{1} + c{2} + c{3}", true).toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
    EXPECT_DOUBLE_EQ(got[0], 61.0);  // {5, 6, 50}: 5 + 6 + 50
}

// G4b: LHS content store c{i} = v with MATLAB auto-grow. c={} then c{1}=..; c{2}=..;
// c{3}=.. builds a cell imperatively (nk_cell_set grows it each time); the three
// contents are read back and summed. Proves the imperative cell-build idiom compiles.
TEST(CodegenBridge, BridgedCellContentStoreRunsAndMatchesInterpreter)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const EmittedFunction emitted =
        emitFunction(*fn, {{"x", InferredType::scalar(ValueType::DOUBLE)}}, reg, nullptr, bridge);
    ASSERT_NE(emitted.source.find("nk_rt::cell_set("), std::string::npos)
        << "c{i}=v must store via the runtime";

    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_cellset_e2e.exe").string();
    const std::string outTxt = (base / "nk_cellset_e2e_out.txt").string();
    std::error_code   ec;
    std::filesystem::remove(outTxt, ec);
    ...
    numkit::StandardEngine engine;
    const double interp =
        engine.eval("c = {}; c{1} = 5; c{2} = 10; c{3} = 105; c{1} + c{2} + c{3}", true).toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
    EXPECT_DOUBLE_EQ(got[0], 120.0);  // {5, 10, 105}: 5 + 10 + 105
}

// G4b: cell-slice c(i) -- PAREN-indexing a cell returns a 1x1 sub-cell (NOT the
// content). It rides the existing A4 Dynamic-index path (index_dyn -> nk_index ->
// indexGet on a cell), so no new emit is needed; this proves it round-trips.
TEST(CodegenBridge, BridgedCellSliceRunsAndMatchesInterpreter)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const EmittedFunction emitted =
        emitFunction(*fn, {{"x", InferredType::scalar(ValueType::DOUBLE)}}, reg, nullptr, bridge);
    ASSERT_NE(emitted.source.find("nk_rt::index_dyn("), std::string::npos)
        << "c(i) cell-slice must ride the Dynamic-index path";

    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_cellslice_e2e.exe").string();
    const std::string outTxt = (base / "nk_cellslice_e2e_out.txt").string();
    std::error_code   ec;
    std::filesystem::remove(outTxt, ec);
    ...
    ASSERT_EQ(got.size(), 1u);
    numkit::StandardEngine engine;
    const double interp = engine.eval("c = {5, 10, 105}; d = c(2); d{1}", true).toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
    EXPECT_DOUBLE_EQ(got[0], 10.0);  // c(2) = {10}; {10}{1} = 10
}

// G5a: a named function handle @funcName created + stored + called in compiled
// code. @abs is a Dynamic FUNC_HANDLE (nk_make_handle); g(x) rides the existing
// Dynamic-index path (index_dyn -> nk_index CALLS the handle). @(params)expr
// closures are refused (capture binding is unsound across the boundary). Diffs
// against the interpreter.
TEST(CodegenBridge, BridgedNamedFunctionHandleRunsAndMatchesInterpreter)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const EmittedFunction emitted =
        emitFunction(*fn, {{"x", InferredType::scalar(ValueType::DOUBLE)}}, reg, nullptr, bridge);
    ASSERT_NE(emitted.source.find("nk_rt::make_handle(\"abs\")"), std::string::npos)
        << "@abs must build a named handle via the runtime";
    ASSERT_NE(emitted.source.find("nk_rt::index_dyn("), std::string::npos)
        << "g(x) must call the handle via the Dynamic-index path";
    ...
    ASSERT_EQ(got.size(), 1u);
    numkit::StandardEngine engine;
    const double interp = engine.eval("g = @abs; g(-5)", true).toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
...
// 2-D content index c{i,j}: read a 2-D cell by (row, col) -- the companion to the
// 2-D literal. {1,2; 3,4}: c{1,1}=1, c{2,1}=3, c{1,2}=2, c{2,2}=4 (sub2indChecked,
// mirroring execCellIndex nidx==2). Diffs vs the interpreter.
TEST(CodegenBridge, Bridged2DCellContentGetRunsAndMatchesInterpreter)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    registerStandardTransfers(reg);
    numkit::Lexer  lex("function r = f(x)\n  c = {x, x + 1; x + 2, x + 3};\n"
                       "  r = c{1,1} * 1000 + c{2,1} * 100 + c{1,2} * 10 + c{2,2};\nend\n");
    numkit::Parser         parser(lex.tokenize());
    auto                   root = parser.parse();
    const numkit::ASTNode *fn   = nullptr;
    for (const auto &ch : root->children)
    ...
    const EmittedFunction emitted =
        emitFunction(*fn, {{"x", InferredType::scalar(ValueType::DOUBLE)}}, reg, nullptr, bridge);
    ASSERT_NE(emitted.source.find("nk_rt::cell_get_2d("), std::string::npos)
        << "c{i,j} must extract via the 2-D content path";

    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_cell2dget_e2e.exe").string();
    const std::string outTxt = (base / "nk_cell2dget_e2e_out.txt").string();
    std::error_code   ec;
    std::filesystem::remove(outTxt, ec);
    ...
    program +=
        ...
        "  std::FILE* gf = std::fopen(\"" + fwd(outTxt) + "\", \"w\");\n"
        "  if (!gf) return 2;\n"
        "  nk_val r = f(1.0);\n"  // {1,2;3,4}: c{1,1}=1,c{2,1}=3,c{1,2}=2,c{2,2}=4 -> 1324
        "  std::fprintf(gf, \"%.17g\\n\", nk_unbox_scalar(r));\n"
        "  nk_release(r);\n"
        "  std::fclose(gf); return 0;\n}\n";
    ...
    numkit::StandardEngine engine;
    const double interp =
        engine.eval("c = {1, 2; 3, 4}; c{1,1} * 1000 + c{2,1} * 100 + c{1,2} * 10 + c{2,2}", true)
            .toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
    EXPECT_DOUBLE_EQ(got[0], 1324.0);  // c{1,1}=1, c{2,1}=3, c{1,2}=2, c{2,2}=4
}

// 2-D content store c{i,j} = v with auto-grow -- the write companion completing
// the cell read/write story. c={} then c{1,1}=..; c{2,1}=..; c{1,2}=..; c{2,2}=..
// grows a 0x0 cell to 2x2 (mirroring resolveCellSlot); read back by (row, col).
TEST(CodegenBridge, Bridged2DCellContentStoreRunsAndMatchesInterpreter)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const EmittedFunction emitted =
        emitFunction(*fn, {{"x", InferredType::scalar(ValueType::DOUBLE)}}, reg, nullptr, bridge);
    ASSERT_NE(emitted.source.find("nk_rt::cell_set_2d("), std::string::npos)
        << "c{i,j}=v must store via the 2-D content path";

    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_cell2dset_e2e.exe").string();
    const std::string outTxt = (base / "nk_cell2dset_e2e_out.txt").string();
    std::error_code   ec;
    std::filesystem::remove(outTxt, ec);
    ...
    program +=
        ...
        "  std::FILE* gf = std::fopen(\"" + fwd(outTxt) + "\", \"w\");\n"
        "  if (!gf) return 2;\n"
        "  nk_val r = f(1.0);\n"  // c{1,1}=1,c{2,1}=2,c{1,2}=3,c{2,2}=4 -> 1234
        "  std::fprintf(gf, \"%.17g\\n\", nk_unbox_scalar(r));\n"
        "  nk_release(r);\n"
        "  std::fclose(gf); return 0;\n}\n";
    ...
    const double interp =
        engine
            .eval("c = {}; c{1,1}=1; c{2,1}=2; c{1,2}=3; c{2,2}=4; "
                  "c{1,1}*1000 + c{2,1}*100 + c{1,2}*10 + c{2,2}",
                  true)
            .toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
    EXPECT_DOUBLE_EQ(got[0], 1234.0);  // c{1,1}=1, c{2,1}=2, c{1,2}=3, c{2,2}=4
}

// G5b: a capture-free anonymous closure @(t) t*t + 1 created, stored, and called in
// compiled code. The body captures nothing (t is the param), so the codegen creates
// it via the interpreter (make_closure -> nk_eval of the func2str-normalized source);
// g(x) rides the Dynamic-index path (index_dyn -> nk_index CALLS the closure). A
// closure that captured an enclosing local would be refused (see the E2E guard).
TEST(CodegenBridge, BridgedCaptureFreeClosureRunsAndMatchesInterpreter)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const EmittedFunction emitted =
        emitFunction(*fn, {{"x", InferredType::scalar(ValueType::DOUBLE)}}, reg, nullptr, bridge);
    ASSERT_NE(emitted.source.find("nk_rt::make_closure(\"@(t)t*t+1\")"), std::string::npos)
        << "the capture-free closure must build from its func2str source";
    ASSERT_NE(emitted.source.find("nk_rt::index_dyn("), std::string::npos)
        << "g(x) must call the closure via the Dynamic-index path";

    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_closure_e2e.exe").string();
    const std::string outTxt = (base / "nk_closure_e2e_out.txt").string();
    std::error_code   ec;
    std::filesystem::remove(outTxt, ec);
...
// max(A,[],dim)/min(A,[],dim) row-/column-wise (phase N51): the explicit-dim 3-arg form. dim 2
// reduces each row (-> m x 1), dim 1 each column (-> 1 x n). Was bridged (3-arg form); now native.
// Bit-exact vs the interpreter.
TEST(CodegenE2E, MaxMinWithDim2D)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const char *body =
        ...
        "  R1 = max(A, [], 1);\n"     // col max -> [4 9 6]
        "  Mn = min(A, [], 2);\n"     // row min -> [1; 1; 4]
        "  r = R2(1) + R2(2)*10 + R2(3)*100 + R1(1)*1000 + Mn(3)*10000 + numel(R2)*100000;\n";
    const EmittedFunction emitted = transpile(
        std::string("function r = f(x)\n") + body + "end\n",
        {{"x", InferredType::concrete(ValueType::DOUBLE, Shape::rowVector())}});
    ...
    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_maxmindim_e2e.exe").string();
    const std::string outTxt = (base / "nk_maxmindim_e2e_out.txt").string();
    std::string       program = emitted.source +
        "#include <cstdio>\n"
        "int main() {\n"
        "  double x[9] = {3, 1, 4, 1, 5, 9, 2, 6, 5};\n"  // 3x3 col-major
        "  double r = f(x, 9);\n"  // 3 + 60 + 900 + 4000 + 40000 + 300000 = 344963
        "  std::FILE* h = std::fopen(\"" + fwd(outTxt) + "\", \"w\");\n"
        "  if (!h) return 2;\n"
        "  std::fprintf(h, \"%.17g\\n\", r);\n"
        "  std::fclose(h); return 0;\n}\n";
    ...
    const std::vector<double> got = compileRunReadDoubles(program, exe, outTxt);
    ASSERT_EQ(got.size(), 1u);
    EXPECT_DOUBLE_EQ(got[0], 344963.0);
    numkit::StandardEngine engine;
    const double interp =
        engine.eval(std::string("x=[3 1 4 1 5 9 2 6 5];\n") + body + "r", true).toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
...
// [s,i]=sort(x) with a permutation-index output (phase N38): sorted values + the 1-based original
// positions. The index output was bridged even for 1-D; now native (stable_sort an index vector,
// so ties keep ascending original order -- MATLAB). Tie in the input checks stability. Bit-exact.
TEST(CodegenE2E, SortWithIndex)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";

    const char *body =
        "  [s, i] = sort(x);\n"            // x=[3 1 4 1 5]: s=[1 1 3 4 5], i=[2 4 1 3 5]
        "  [sd, id] = sort(x, 'descend');\n"  // sd=[5 4 3 1 1], id=[5 3 1 2 4]
        "  r = s(1) + i(1)*10 + s(5)*100 + i(5)*1000 + sd(1)*10000 + id(1)*100000"
        " + numel(s)*1000000;\n";
    const EmittedFunction emitted = transpile(
        std::string("function r = f(x)\n") + body + "end\n",
        {{"x", InferredType::concrete(ValueType::DOUBLE, Shape::rowVector())}});
    ...
    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_sortidx_e2e.exe").string();
    const std::string outTxt = (base / "nk_sortidx_e2e_out.txt").string();
    std::string       program = emitted.source +
        "#include <cstdio>\n"
        "int main() {\n"
        "  double x[5] = {3, 1, 4, 1, 5};\n"  // tie: two 1s at positions 2 and 4
        "  double r = f(x, 5);\n"  // 1 + 20 + 500 + 5000 + 50000 + 500000 + 5000000 = 5555521
        "  std::FILE* h = std::fopen(\"" + fwd(outTxt) + "\", \"w\");\n"
        "  if (!h) return 2;\n"
        "  std::fprintf(h, \"%.17g\\n\", r);\n"
        "  std::fclose(h); return 0;\n}\n";
    ...
    const std::vector<double> got = compileRunReadDoubles(program, exe, outTxt);
    ASSERT_EQ(got.size(), 1u);
    EXPECT_DOUBLE_EQ(got[0], 5555521.0);
    numkit::StandardEngine engine;
    const double interp =
...
// 2-D [s,i]=sort(A) per-column (phase N39): each column sorts independently; s = sorted columns,
// i = the per-column 1-based permutation. The multi-output sort was 1-D only -> a matrix form was
// bridged; now native (per-column stable_sort of an index vector). Bit-exact vs the interpreter.
TEST(CodegenE2E, SortWithIndex2D)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const char *body =
        "  A = reshape(x, 3, 2);\n"  // [3 6; 1 4; 2 5]
        "  [s, i] = sort(A);\n"      // s=[1 4; 2 5; 3 6], i=[2 2; 3 3; 1 1]
        "  r = s(1,1) + s(3,1)*10 + i(1,1)*100 + i(2,1)*1000 + s(1,2)*10000 + i(3,2)*100000"
        " + numel(s)*1000000;\n";
    const EmittedFunction emitted = transpile(
        std::string("function r = f(x)\n") + body + "end\n",
        {{"x", InferredType::concrete(ValueType::DOUBLE, Shape::rowVector())}});
    ...
    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_sortidx2d_e2e.exe").string();
    const std::string outTxt = (base / "nk_sortidx2d_e2e_out.txt").string();
    std::string       program = emitted.source +
        "#include <cstdio>\n"
        "int main() {\n"
        "  double x[6] = {3, 1, 2, 6, 4, 5};\n"  // 3x2 col-major
        "  double r = f(x, 6);\n"  // 1 + 30 + 200 + 3000 + 40000 + 100000 + 6000000 = 6143231
        "  std::FILE* h = std::fopen(\"" + fwd(outTxt) + "\", \"w\");\n"
        "  if (!h) return 2;\n"
        "  std::fprintf(h, \"%.17g\\n\", r);\n"
        "  std::fclose(h); return 0;\n}\n";
    ...
    const std::vector<double> got = compileRunReadDoubles(program, exe, outTxt);
    ASSERT_EQ(got.size(), 1u);
    EXPECT_DOUBLE_EQ(got[0], 6143231.0);
    numkit::StandardEngine engine;
    const double interp =
        engine.eval(std::string("x=[3 1 2 6 4 5];\n") + body + "r", true).toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
...
// 2-D [u,ia,ic]=unique(A) (phase N41): unique flattens the matrix column-major -> u sorted distinct,
// ia the first-occurrence LINEAR index, ic the map (length numel). Multi-output unique was 1-D
// only; now native for a matrix too. Repeats across the flattened buffer. Bit-exact vs interp.
TEST(CodegenE2E, UniqueWithIndices2D)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";

    const char *body =
        "  A = reshape(x, 2, 3);\n"     // flat [30 10 40 10 50 10]
        "  [u, ia, ic] = unique(A);\n"  // u=[10 30 40 50], ia=[2 1 3 5], ic=[2 1 3 1 4 1]
        "  r = u(1) + u(2)*10 + ia(1)*100 + ia(2)*1000 + ic(1)*10000 + numel(u)*100000"
        " + numel(ic)*1000000;\n";
    const EmittedFunction emitted = transpile(
        std::string("function r = f(x)\n") + body + "end\n",
        {{"x", InferredType::concrete(ValueType::DOUBLE, Shape::rowVector())}});
    ...
    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_uniqueidx2d_e2e.exe").string();
    const std::string outTxt = (base / "nk_uniqueidx2d_e2e_out.txt").string();
    std::string       program = emitted.source +
        "#include <cstdio>\n"
        "int main() {\n"
        "  double x[6] = {30, 10, 40, 10, 50, 10};\n"  // 2x3 col-major, repeats of 10
        "  double r = f(x, 6);\n"  // 10 + 300 + 200 + 1000 + 20000 + 400000 + 6000000 = 6421510
        "  std::FILE* h = std::fopen(\"" + fwd(outTxt) + "\", \"w\");\n"
        "  if (!h) return 2;\n"
        "  std::fprintf(h, \"%.17g\\n\", r);\n"
        "  std::fclose(h); return 0;\n}\n";
    ...
    const std::vector<double> got = compileRunReadDoubles(program, exe, outTxt);
    ASSERT_EQ(got.size(), 1u);
    EXPECT_DOUBLE_EQ(got[0], 6421510.0);
    numkit::StandardEngine engine;
    const double interp =
...
// [c,ia]=setdiff(a,b) (phase N46): c = sorted distinct of a not in b, ia the first-occurrence index
// in a (c=a(ia)). Was bridged; now native (stable index sort of a + membership filter, like unique-
// multi so ia is correct including duplicate NaNs). Bit-exact vs the interpreter.
TEST(CodegenE2E, SetdiffWithIndices)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const char *body =
        "  b = [20 40 10 40];\n"
        "  [c, ia] = setdiff(x, b);\n"  // x=[30 10 40 10 50]: c=[30 50], ia=[1 5]
        "  r = c(1) + c(2)*10 + ia(1)*100 + ia(2)*1000 + numel(c)*10000;\n";
    const EmittedFunction emitted = transpile(
        std::string("function r = f(x)\n") + body + "end\n",
        {{"x", InferredType::concrete(ValueType::DOUBLE, Shape::rowVector())}});
    ...
    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_setdiffidx_e2e.exe").string();
    const std::string outTxt = (base / "nk_setdiffidx_e2e_out.txt").string();
    std::string       program = emitted.source +
        "#include <cstdio>\n"
        "int main() {\n"
        "  double x[5] = {30, 10, 40, 10, 50};\n"
        "  double r = f(x, 5);\n"  // c=[30 50], ia=[1 5]: 30 + 500 + 100 + 5000 + 20000 = 25630
        "  std::FILE* h = std::fopen(\"" + fwd(outTxt) + "\", \"w\");\n"
        "  if (!h) return 2;\n"
        "  std::fprintf(h, \"%.17g\\n\", r);\n"
        "  std::fclose(h); return 0;\n}\n";
    ...
    const std::vector<double> got = compileRunReadDoubles(program, exe, outTxt);
    ASSERT_EQ(got.size(), 1u);
    EXPECT_DOUBLE_EQ(got[0], 25630.0);
    numkit::StandardEngine engine;
    const double interp =
        engine.eval(std::string("x=[30 10 40 10 50];\n") + body + "r", true).toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
...
// rank-3 diff along dim 1 or 2 (phase N58): diff(A,1,1) -> [m-1,n,p], diff(A,1,2) -> [m,n-1,p]. The
// diff producer was 1-D + 2-D only -> a rank-3 diff was bridged; now native for the NON-trailing
// dims (the reduced dim is non-trailing so the rank is preserved). diff(A,1,3) stays SOUND-REFUSED
// (it can collapse the trailing dim to extent 1 = MATLAB rank-2). Non-monotone input. Bit-exact vs
// the interpreter.
TEST(CodegenE2E, DiffRank3Dim1Dim2)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const char *body =
        "  A = reshape(x, 2, 2, 3);\n"   // pages [4 7;2 1], [3 6;8 5], [9 1;2 7]
        "  D = diff(A, 1, 1);\n"         // [1,2,3]: D(1,1,1)=2-4=-2, D(1,2,3)=7-1=6
        "  E = diff(A, 1, 2);\n"        // [2,1,3]: E(1,1,1)=7-4=3, E(2,1,1)=1-2=-1, E(2,1,3)=7-2=5
        "  r = D(1,1,1) + D(1,2,3)*10 + E(1,1,1)*100 + E(2,1,1)*1000 + E(2,1,3)*10000"
        " + numel(D)*100000;\n";
    const EmittedFunction emitted = transpile(
        std::string("function r = f(x)\n") + body + "end\n",
        {{"x", InferredType::concrete(ValueType::DOUBLE, Shape::rowVector())}});
    ...
    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_diffrank3_e2e.exe").string();
    const std::string outTxt = (base / "nk_diffrank3_e2e_out.txt").string();
    std::string       program = emitted.source +
        "#include <cstdio>\n"
        "int main() {\n"
        "  double x[12] = {4, 2, 7, 1, 3, 8, 6, 5, 9, 2, 1, 7};\n"  // 2x2x3 col-major
        "  double r = f(x, 12);\n"  // -2 + 60 + 300 - 1000 + 50000 + 600000 = 649358
        "  std::FILE* h = std::fopen(\"" + fwd(outTxt) + "\", \"w\");\n"
        "  if (!h) return 2;\n"
        "  std::fprintf(h, \"%.17g\\n\", r);\n"
        "  std::fclose(h); return 0;\n}\n";
    ...
    const std::vector<double> got = compileRunReadDoubles(program, exe, outTxt);
    ASSERT_EQ(got.size(), 1u);
    EXPECT_DOUBLE_EQ(got[0], 649358.0);
    numkit::StandardEngine engine;
    const double interp =
        engine.eval(std::string("x=[4 2 7 1 3 8 6 5 9 2 1 7];\n") + body + "r", true).toScalar();
    EXPECT_DOUBLE_EQ(got[0], interp);
...
// Struct RETURN across the ABI (G2.4): a function returning a struct fills the caller's
// per-field locals via out-param refs (the inverse of the struct param explosion). v1: all-
// scalar fields. mk() returns {a:5, b:7}; g binds them into t and reads t.a + t.b*10 = 75.
TEST(CodegenE2E, StructReturnAcrossABI)
{
    if (!aot::available())
        GTEST_SKIP() << "no external compiler configured for this build";
    ...
    const EmittedFunction emitted = emitProgram(*ft.find("g"), {}, ft, reg);
    EXPECT_NE(emitted.source.find("_nk_fld_t_a"), std::string::npos)
        << "the struct-returning call must bind the caller's per-field locals";

    auto base = std::filesystem::temp_directory_path() / "numkit_codegen_aot";
    std::filesystem::create_directories(base);
    const std::string exe    = (base / "nk_structret_e2e.exe").string();
    const std::string outTxt = (base / "nk_structret_e2e_out.txt").string();
    std::string       program = emitted.source +
        "#include <cstdio>\n"
        "int main() {\n"
        "  double out = " + emitted.name + "();\n"  // 5 + 7*10 = 75
