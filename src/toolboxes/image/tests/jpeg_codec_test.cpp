...
// Self-contained test for JPEG with DRI (Define Restart Interval 0xDD) marker
// and RST0..RST7 restart markers between MCU blocks.
TEST(JpegCodecTest, RestartIntervalDriAndRstMarkers) {
    // A valid 16x16 grayscale JPEG with DRI=1 and RST markers
    // (2x2 MCU grid of 8x8 blocks, separated by RST0, RST1, RST2).
    const uint8_t kJpegDriSample[] = {
        ...
        0xFF, 0xDD, 0x00, 0x04, 0x00, 0x01, // DRI marker: restart interval = 1 MCU
        ...
        0xFF, 0xD2,
        // MCU 3
        0xB4, 0x68, 0xE9, 0xF7, 0x6B,
        0xFF, 0xD9 // EOI
    };

    uint32_t w = 0, h = 0;
    uint16_t bits = 0, ch = 0;
    EXPECT_TRUE(peekJpeg(kJpegDriSample, sizeof(kJpegDriSample), w, h, bits, ch));
    EXPECT_EQ(w, 16u);
    EXPECT_EQ(h, 16u);
    EXPECT_EQ(bits, 8u);
    EXPECT_EQ(ch, 1u);

    Value v = readJpeg(kJpegDriSample, sizeof(kJpegDriSample));
    EXPECT_EQ(v.dims().rows(), 16u);
    EXPECT_EQ(v.dims().cols(), 16u);
    EXPECT_EQ(v.type(), ValueType::UINT8);

    // Verify non-zero decoded pixel intensity
    double sum = 0;
    for (size_t i = 0; i < v.numel(); ++i) {
        sum += v.uint8Data()[i];
    }
    EXPECT_GT(sum / v.numel(), 1.0);
}
