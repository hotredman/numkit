...
#include "elmat/matrix_detail.hpp"
...
void reshape_reg(Span<const Value> args, size_t /*nargout*/, Span<Value> outs, CallContext &ctx)
{
    ...
    outs[0] = reshape(x, Span<const size_t>(dims.data(), dims.size()), mr);
}
...
NK_LOGICAL_RED_REG(any, numkit::builtin::anyOf)
NK_LOGICAL_RED_REG(all, numkit::builtin::allOf)
