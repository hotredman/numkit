...
TEST(DiscreteSetopsPublicApi, SetxorAllNumUnique)
{
    ...
    Value c = numkit::builtin::setxor(sv(e, "[1 2 3]", "a"), sv(e, "[2 3 4]", "b"));
    ASSERT_EQ(c.numel(), 2u);
    EXPECT_DOUBLE_EQ(c.doubleData()[0], 1.0);
    EXPECT_DOUBLE_EQ(c.doubleData()[1], 4.0);
    // allunique
    EXPECT_NE(numkit::builtin::allunique(sv(e, "[1 2 3]", "u1"), e.resource()).toScalar(),
              0.0);
    EXPECT_EQ(numkit::builtin::allunique(sv(e, "[1 2 2]", "u2"), e.resource()).toScalar(),
              0.0);
    // numunique
    EXPECT_DOUBLE_EQ(
        numkit::builtin::numunique(sv(e, "[1 2 2 3 3 3]", "n"), e.resource()).toScalar(),
        3.0);
}
...
TEST(DiscreteSetopsPublicApi, IsmemberUniqueTol)
{
    ...
    Value tf = numkit::builtin::ismembertol(sv(e, "[1 5]", "q"), sv(e, "[1 2 3]", "s"));
    ASSERT_EQ(tf.numel(), 2u);
    EXPECT_NE(tf.elemAsDouble(0), 0.0);
    EXPECT_EQ(tf.elemAsDouble(1), 0.0);
    // explicit tol argument
    Value tf2 = numkit::builtin::ismembertol(sv(e, "[2]", "q2"), sv(e, "[1 2 3]", "s2"),
                                     1e-6, e.resource());
    EXPECT_NE(tf2.elemAsDouble(0), 0.0);
    // uniquetol collapses exact duplicates -> [1 2 3]
    Value u = numkit::builtin::uniquetol(sv(e, "[1 2 2 3]", "x"), 1e-6, e.resource());
