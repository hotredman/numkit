...
TEST(ManipPublicApi, Sub2ind)
{
    ...
    Value lin = numkit::builtin::sub2ind(siz, Span<const Value>(subs1)); // mr defaulted
    EXPECT_DOUBLE_EQ(lin.toScalar(), 8.0);
    // vector subscripts -> [1 2 6]
    std::vector<Value> subs2 = {mvar(e, "[1 2 3]", "rr"), mvar(e, "[1 1 2]", "cc")};
    Value linv = numkit::builtin::sub2ind(siz, Span<const Value>(subs2), e.resource());
    ...
    EXPECT_ANY_THROW(
        numkit::builtin::sub2ind(mvar(e, "[]", "emptysz"), Span<const Value>(s)));
}
...
TEST(ManipPublicApi, Ind2sub)
{
    ...
    std::vector<Value> rs = numkit::builtin::ind2sub(siz, mvar(e, "8", "ind"));
    ASSERT_EQ(rs.size(), 2u);
    EXPECT_DOUBLE_EQ(rs[0].toScalar(), 2.0);
    EXPECT_DOUBLE_EQ(rs[1].toScalar(), 3.0);
    // vector ind [1 2 6], explicit nout = 2
    std::vector<Value> rv =
        numkit::builtin::ind2sub(siz, mvar(e, "[1 2 6]", "iv"), 2, e.resource());
    ...
    Value back = numkit::builtin::sub2ind(siz, Span<const Value>(rv), e.resource());
    EXPECT_DOUBLE_EQ(back.doubleData()[2], 6.0);
    // empty siz throws
    EXPECT_ANY_THROW(numkit::builtin::ind2sub(mvar(e, "[]", "esz"), mvar(e, "1", "i1")));
}
