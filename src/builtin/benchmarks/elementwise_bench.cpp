...
#include <numkit/builtin/elfun.hpp>
#include <numkit/builtin/elfun.hpp>
#include <numkit/builtin/datatypes.hpp>        // isfinite (lang API, math SIMD kernel)
...
static void BM_Tan(benchmark::State &s)      { runElementwiseBench(s, numkit::builtin::tan,      -10.0, 10.0);  }
static void BM_Sqrt(benchmark::State &s)     { runElementwiseBench(s, numkit::builtin::sqrt,       0.0, 100.0); }
static void BM_Round(benchmark::State &s)    { runElementwiseBench(s, numkit::builtin::round,    -1e6,  1e6);   }
static void BM_Floor(benchmark::State &s)    { runElementwiseBench(s, numkit::builtin::floor,    -1e6,  1e6);   }
static void BM_Ceil(benchmark::State &s)     { runElementwiseBench(s, numkit::builtin::ceil,     -1e6,  1e6);   }
static void BM_Fix(benchmark::State &s)      { runElementwiseBench(s, numkit::builtin::fix,      -1e6,  1e6);   }
static void BM_Isfinite(benchmark::State &s) { runElementwiseBench(s, numkit::builtin::isfinite, -1e6,  1e6);   }
