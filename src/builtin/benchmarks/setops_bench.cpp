...
static void BM_Unique(benchmark::State &s)
{
    ...
    for (auto _ : s) {
        auto y = numkit::builtin::unique(x, mr);
...
static void BM_Ismember(benchmark::State &s)
{
    ...
    std::pmr::memory_resource *mr = std::pmr::get_default_resource();
    for (auto _ : s) {
        auto y = numkit::builtin::ismember(a, b, mr);
        benchmark::DoNotOptimize(y);
    }
    s.SetItemsProcessed(s.iterations() * static_cast<int64_t>(n));
}
...
static void BM_Union    (benchmark::State &s) { runPairBench(s, [](auto &a, auto &x, auto &y){ return numkit::builtin::setUnion    (x, y, a); }); }
static void BM_Intersect(benchmark::State &s) { runPairBench(s, [](auto &a, auto &x, auto &y){ return numkit::builtin::setIntersect(x, y, a); }); }
static void BM_Setdiff  (benchmark::State &s) { runPairBench(s, [](auto &a, auto &x, auto &y){ return numkit::builtin::setDiff     (x, y, a); }); }
...
static void BM_Histcounts(benchmark::State &s)
{
    ...
    std::pmr::memory_resource *mr = std::pmr::get_default_resource();
    for (auto _ : s) {
        auto y = numkit::builtin::histcounts(x, edges, mr);
        benchmark::DoNotOptimize(y);
    }
    s.SetItemsProcessed(s.iterations() * static_cast<int64_t>(n));
}
