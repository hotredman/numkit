...
function TemporaryBrowser({ onOpenFile, onRefreshKey, isTabUnsaved }) {
  ...
  const [expanded, setExpanded] = usePersistedState('numkit.ide.fb.expanded.temporary', {});
...
function LocalFolderBrowser({ onOpenFile, isTabUnsaved, onLocalMount, onRefreshKey }) {
  ...
  const [expanded, setExpanded] = usePersistedState('numkit.ide.fb.expanded.localFolder', {});
  const [selected, setSelected] = useState(null);
  const [contextMenu, setContextMenu] = useState(null);
  const [creating, setCreating] = useState(null);
  const [renaming, setRenaming] = useState(null);
...
function ExamplesBrowser({ onOpenFile, vfsAdapters }) {
  const C = useTheme();
  const [tree, setTree] = useState([]);
  const [expanded, setExpanded] = usePersistedState('numkit.ide.fb.expanded.examples', {});
  const [selected, setSelected] = useState(null);
...
function GitHubBrowser({ onOpenFile, defaultRepo }) {
  ...
  const [selected, setSelected] = useState(null);
  const [branch, setBranch] = usePersistedState('numkit.ide.fb.github.branch', 'main');
...
export default function FileBrowser({ onOpenFile, defaultGitHubRepo, vfsRefreshKey, isTabUnsaved, onLocalMount, vfsAdapters }) {
  ...
  const [source, setSource] = usePersistedState('numkit.ide.fb.source', 'temporary', {
